...
theta_0 = np.array([[np.nan, 1, 1, np.nan],       # s0
                    ...
                    [1, 1, np.nan, np.nan],       # s7, s8はゴールなので方策はなし
                    ])
...
def get_next_s(pi, s):
    ...
    print("s : {}".format(s))
    print("pi : {}".format(pi))
    print("pi[s, :].shape : {}".format(pi[s, :].shape))
    next_direction = np.random.choice(a=direction, p=pi[s, :])
